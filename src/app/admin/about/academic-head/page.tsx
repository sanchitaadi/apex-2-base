...
const defaultData: AcademicHead = {
  slug: "academic-head",
...
export default function AdminAcademicHeadPage() {
  const [form, setForm] =
    useState<AcademicHead>(defaultData);
  ...
  const setField = <K extends keyof AcademicHead>(
    key: K,
    value: AcademicHead[K]
  ) => {
    setForm((current) => ({
      ...current,
      [key]: value,
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (error) {
        console.error(error);
        setMessage(
          "Unable to load Academic Head content."
        );
        setLoading(false);
        return;
      }
  ...
  const uploadImage = async (
    ...
    try {
      ...
      const path =
        `academic-head/${Date.now()}.${extension}`;
  ...
  const save = async (event: FormEvent) => {
    ...
    setMessage(
      "Academic Head page saved successfully."
    );
        ...
        <div className="mb-8 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          ...
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-[#10203A]/60">
              Manage the Academic Head profile, message,
              focus areas, image, contact buttons and
              navigation.
            </p>
        ...
        <form onSubmit={save} className="space-y-6">
          ...
          <section className="rounded-3xl bg-white p-6 shadow-sm md:p-8">
            ...
            <div className="mb-6 flex items-center justify-between gap-5">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-[#10203A]/55">
                  Manage the two focus cards shown below the message.
                </p>
              </div>
          ...
          <div className="sticky bottom-5 z-20 flex flex-col gap-3 rounded-2xl border border-[#102A56]/10 bg-[#F5F0E6]/95 p-4 backdrop-blur md:flex-row md:items-center md:justify-between">
            ...
            <p className="text-sm text-[#10203A]/60">
              {message ||
                "Changes are saved to the Academic Head CMS."}
            </p>
